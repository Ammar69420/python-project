...
def generate_key(msg, key):
    ...
    key_length = len(key)
    key_as_list = []
    msg_index = 0  # Index for the original message

    for i in range(len(msg)):
        if msg[i].isalpha():  # Only consider alphabetic characters
            key_as_list.append(key[msg_index % key_length])
            msg_index += 1
        else:
            key_as_list.append(msg[i])  # Keep non-alphabetic characters unchanged
